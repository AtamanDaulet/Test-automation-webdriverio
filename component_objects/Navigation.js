import Page from './Page'

class Navigation extends Page {

    get navBar() {
        return $('.site-content > div > div:nth-child(1) > div');
    }
    get newBug() {
        return $('#new_bug');
    }
    get all_issues() {
        return $('#all_issues');
    }
    get logout() {
        return $('#logout');
    }
    get header() {
        return $('[class="card-footer text-center custom-header"]');
    }
    get headerSiblingNavBar() {
        return $('[class="card-footer text-center custom-header"]')
            .firstChild; //'[class=card-footer text-center]'
    }
    // get searchBar()  { 
    //     return $('#twotabsearchtextbox')
    // }
    // get submitSearch()  { 
    //     return $('input.nav-input')
    // }
    // get shoppingCart()  { 
    //     return $('#nav-cart')
    // }
    // get hamburger()  { 
    //   return $('#nav-hamburger-menu')
    // }
    // get expandedNav()  { 
    //   return $('#hmenu-canvas')
    // }
    // get closeExpandedNav()  { 
    //   return $('[class="nav-sprite hmenu-close-icon"]')
    // }
    // get signInFlyout()  { 
    //   return $('[class="nav-signin-tt nav-flyout"]')
    // }
    // get signInTooltip()  { 
    //   return $('#nav-signin-tooltip')
    // }
    // get signInTooltipButton()  { 
    //   return $('[data-nav-ref="nav_custrec_signin"]')
    // }
    // get signInTooltipLink()  { 
    //   return $('#nav-signin-tooltip > div > a')
    // }

    open(path) {
        super.open(path)
    }
}

export default new Navigation()